...
class TelegramBot:
    ...
    def __init__(self,
        ...
        def generate_post(message: telebot.types.Message):
        ...
        def new_word(message: telebot.types.Message):
            if str(message.from_user.id) not in ['466788660', '863047444']:
                self.bot.send_message(message.chat.id, "Вы не являетесь администратором!")
                return
        ...
        def delete_word(message: telebot.types.Message):
            if str(message.from_user.id) not in ['466788660', '863047444']:
                self.bot.send_message(message.chat.id, "Вы не являетесь администратором!")
                return
        ...
        def list_words(message: telebot.types.Message):
            if str(message.from_user.id) not in ['466788660', '863047444']:
                self.bot.send_message(message.chat.id, "Вы не являетесь администратором!")
                return
        ...
        def delete_bot_bd(message: telebot.types.Message):
            db.delete_bot(-1)
            self.bot.send_message(message.chat.id, 'Бот удален')

        #Функция запроса gpt
        @self.bot.message_handler()
        def generate_answer(message: telebot.types.Message):
            if str(message.from_user.id) not in ['466788660', '863047444']:
                self.bot.send_message(message.chat.id, "Вы не являетесь администратором!")
                return
    ...
    def generate_post_new_threading(self, message: telebot.types.Message):
        while not self.stop_event.is_set():
            ...
            for i in range(1, num_photo):
                ...
                media_group.append(
                    telebot.types.InputMediaPhoto(open(cwd / f"images/ImageBot{self.number}/ImageGen{i}.png", 'rb'), caption=text if i == 1 else ''))
    ...
    def add_new_word_txt(self, message: telebot.types.Message):
