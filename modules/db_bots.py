...
# Декоратор для проверки существуют ли бот
def validator(func):
...
@validator
def bots_database(id_chat, name, api, request, time):
...
def get_all_info_one_bot(id_bot):
    with open(cwd / 'data/db_bots/database_bots.txt', 'r', encoding='utf-8') as file:
        word = file.readlines()
    return word[id_bot]
